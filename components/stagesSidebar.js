  ...
  const selectStage = (s, job) => {
    ...
    return(
        ...
        <div className="flex flex-col font-extrabold flex-shrink w-48 p-2 shadow-inner  ">
          <p className="font-black p-2 text-xl">Stages ({stages?.length})</p>
        
            {stages && stages.map(stage => {
               if( (id != stage.id)) { return <button className="flex font-bold flex-row p-2 justify-between hover:bg-blue-50 cursor-pointer rounded-md" onClick={() => selectStage(stage, job)}>
...
{stage.name != "Recrutement" && stage.name != "Réception"  && <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
  <path strokeLinecap="round" strokeLinejoin="round" d="M3 7v10a2 2 0 002 2h14a2 2 0 002-2V9a2 2 0 00-2-2h-6l-2-2H5a2 2 0 00-2 2z" />
                    ...
                    if((id == stage.id)) {return <div className="flex flex-row p-2 justify-between items-center bg-blue-700 text-white cursor-pointer rounded-md" onClick={() => selectStage(stage, job)}>
...
{stage.name != "Recrutement" && stage.name != "Réception"  && <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
  <path strokeLinecap="round" strokeLinejoin="round" d="M3 7v10a2 2 0 002 2h14a2 2 0 002-2V9a2 2 0 00-2-2h-6l-2-2H5a2 2 0 00-2 2z" />
            ...
            {job.setup ? <div className="flex mt-8 p-2 mr-2  flex-row justify-start bg-blue-700 text-white cursor-pointer rounded-md" onClick={() => router.push(`/jobs/${job.job.id}/setup/details`)}>
