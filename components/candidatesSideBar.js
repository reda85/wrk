...
export default function CandidatesSideBar( job) {
  ...
  console.log("query", router.query)
    ...
    return(
        ...
        {loading && <LoadingOutlined className='h-5 w-5 text-indigo-500' />}
         {!loading && <div>   <div className="flex flex-row p-3  justify-between">
            {stages && stages.length > 0 && <span className="text-indigo-500 font-extrabold text-lg ">{stages.filter(stage => stage.id == id)[0].name}</span> } 
  ...
  <path strokeLinecap="round" strokeLinejoin="round" d="M12 5v.01M12 12v.01M12 19v.01M12 6a1 1 0 110-2 1 1 0 010 2zm0 7a1 1 0 110-2 1 1 0 010 2zm0 7a1 1 0 110-2 1 1 0 010 2z" />
            ...
            {mycandidates && mycandidates.length >0 && mycandidates.map(candidate => {
              ...
              console.log('ghghgh')
              if((applicantId !== candidate.id)) { return <div className=" font-bold text-gray-500 group flex flex-row cursor-pointer items-center justify-between hover:bg-indigo-50 p-2 rounded-md" onClick={() => selectCandidate(candidate)}>
  ...
  <path strokeLinecap="round" strokeLinejoin="round" d="M9 5l7 7-7 7" />
                ...
                if((applicantId === candidate.id)) {  return <div className=" items-center font-bold group flex flex-row cursor-pointer  justify-between bg-indigo-500 text-white p-2 rounded-md" onClick={() => selectCandidate(candidate)}>
...
<path strokeLinecap="round" strokeLinejoin="round" d="M9 5l7 7-7 7" />
                ...
                else{ return <div className=" group flex flex-row cursor-pointer items-center justify-between hover:bg-indigo-50 p-2 rounded-md" onClick={() => selectCandidate(candidate)}>
